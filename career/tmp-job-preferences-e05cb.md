...
- プログラミング言語、フレームワーク、ツールやサービス
  ...
  - 即戦力で入って最速で成果出すなら、サーバーサイドは python(Django, FastAPI, Streamlit あたりはスラスラ書ける), 2 番手で TypeScript(Node.js)。Rust もまあまあ書いてる。フロントエンドも React/Next.js ならそこそこ読み書きできるつもり。C/C++/Ruby/Java/Scala はちょっとできる。クラウドは AWS メイン。GCP も本番環境の運用経験あるけどブランクあり、最近はあまり深くは使いこなせていない。
